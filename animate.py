...
POMONA_WATCHER_DIR = os.path.expandvars("${HOME}/pomona-watcher")
CAPTURES_DIR = os.path.join(POMONA_WATCHER_DIR, "static/captures")
VIDEOS_DIR = os.path.join(POMONA_WATCHER_DIR, "static/videos")
...
try:
    ...
    TEMP_DIR = os.path.join(POMONA_WATCHER_DIR, "static/temp")
    ...
    title = "Pomona - {}".format(datetime.date.today().strftime('%A %-e %B %Y'))

    # Upload to YouTube
    os.system("python {}/upload_video.py --noauth_local_webserver --file='{}/{}' --title='{}' --description='{}' --category={}".format(
        POMONA_WATCHER_DIR, CAPTURES_DIR, filename, title, desc, category))
